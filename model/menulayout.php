<?php

echo '
<li><a href="tomadeturnos"><i class="fa fa-clock-o"></i> Toma de Turnos.</a></li>
<li><a href="download"><i class="fa fa-cloud-download"></i> Descargar&nbsp Planilla.</a></li>
<li><a href="gift"><i class="fa fa-gift"></i> Regalar T. 17:30/19:00.</a></li>
 <li><a href="account"><i class="fa fa-cogs"></i> Cuenta.</a></li>
<li><a href="contact"><i class="fa fa-envelope-o"></i> Contact.</a></li>
<li>&nbsp;</li>
<li> <a href="#"><i class="fa fa-wrench"></i> Gestionar..</a>
<ul>
<li><a href="manage"><i class="fa fa-users"></i> Administrar</a></li>
<li><a href="Layoff"><i class="fa fa-lock"></i> Suspenciones</a></li>
<li><a href="resize"><i class="fa fa-table"></i> Redimencionar</a></li>
</ul>
</li>';

?>

